...
function genesis_extender_custom_import( $import_file, $theme_settings = '', $custom_css = '', $custom_functions = '', $custom_js = '', $custom_templates = '', $custom_labels = '', $conditionals = '', $widget_areas = '', $hook_boxes = '', $images = '' )
{
	...
	if( 'zip' == strtolower( substr( strrchr( $import_file['name'], '.' ), 1 ) ) )
	{
		...
		wp_redirect( admin_url( 'admin.php?page=genesis-extender-dashboard&activetab=genesis-extender-settings-nav-import-export&notice=import-complete' ) );
		exit();
	}
	...
	{
		wp_redirect( admin_url( 'admin.php?page=genesis-extender-dashboard&activetab=genesis-extender-settings-nav-import-export&notice=import-error' ) );
		exit();
	}
}
