...
function genesis_extender_build_google_fonts_string()
{
	...
	foreach( $genesis_extender_google_font_array as $google_font => $google_font_data )
	{
		...
		if( $google_font_data['subset'] != '' )
			$subsets .= $google_font_data['subset'] . ',';
	}
	
	if( !empty( $google_fonts ) && $subsets != '' )
	{
		$subsets = implode( ',', array_unique( explode( ',', $subsets ) ) );
		return rtrim( $google_fonts, '|' ) . '&subset=' . rtrim( $subsets, ',' );
	}
	elseif( !empty( $google_fonts ) && $subsets == '' )
		return rtrim( $google_fonts, '|' );
	else
		return false;
}
...
function genesis_extender_enqueue_google_fonts()
{
	...
	$theme_version = defined( 'CHILD_THEME_VERSION' ) ? CHILD_THEME_VERSION : PARENT_THEME_VERSION;
	
	if( !empty( $google_fonts ) )
	{
		$google_fonts_enqueue = wp_enqueue_style( 'genesis-extender-google-fonts', '//fonts.googleapis.com/css?family=' . $google_fonts, array(), $theme_version );
	}
...
function genesis_extender_font_types_array( $type = 'all' )
{
	...
	foreach( $current_google_fonts_array as $current_google_font )
	{
		$google_font_pieces = explode( '|', $current_google_font );
  		...
  		$subset = '';
		if( ( $pos = strpos( $google_font_pieces[1], '=' ) ) !== FALSE )
		    $subset = substr( $google_font_pieces[1], $pos + 1 ); 
