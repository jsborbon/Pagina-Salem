...
add_action( 'admin_init', 'genesis_extender_sl_plugin_updater', 0 );
/**
...
function genesis_extender_sl_plugin_updater()
{
	...
	$edd_updater = new EDD_SL_Plugin_Updater( GENEXT_COBALT_APPS_URL, GENEXT_PATH . 'genesis-extender.php', array( 
			'version' 	=> GENEXT_VERSION, 				// current version number
			...
			<form method="post" action="options.php">
				...
				<div class="bg-box">
					<p>
						...
						<input id="genesis_extender_license_key" name="genesis_extender_license_key" type="password" class="regular-text" value="<?php echo esc_attr_e( $license ); ?>" style="width:100%; max-width:300px;"/>
...
function genesis_extender_activate_license()
{
	...
	if( isset( $_POST['genesis_extender_license_activate'] ) )
	{
		...
		// Call the custom API.
		$response = wp_remote_post( GENEXT_COBALT_APPS_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
