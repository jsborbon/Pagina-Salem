...
class Layout_CPT_Archive extends Layout_Blog {
		...
		$this->add_section(
			new Section(
				...
				array(
					...
					'title'    => $this->model->get_plural() . ' / ' . esc_html__( 'Archive', 'neve' ),
					'panel'    => 'neve_layout',
				)
			)
		);
		...
		$this->add_control(
			new Control(
				...
				[
					...
					'description'     => esc_html__( 'By default the settings are inherited from Blog layout.', 'neve' ),
	...
	private function add_layout_controls() {
		...
		$this->add_control(
			new Control(
				...
				array(
					'label'            => $this->model->get_plural() . ' ' . esc_html__( 'Layout', 'neve' ),
					'section'          => $this->section,
					'priority'         => 10,
		 ...
		 * Grid Layout Control
		 */
		$grid_default         = neve_is_new_skin() ? '{"desktop":3,"tablet":2,"mobile":1}' : '{"desktop":1,"tablet":1,"mobile":1}';
		...
		$this->add_control(
			new Control(
				...
				[
					...
					'active_callback' => function () {
						return $this->model->is_custom_layout_archive_enabled() && ( Mods::get( $this->section ) === 'default' );
					},
				]
			)
		);
	...
	private function add_post_meta_controls() {
		...
		if ( $has_custom_meta ) {
			...
			$default_value     = Layout::get_meta_default_data( 'neve_' . $this->model->get_archive_type() . '_post_meta_ordering', $order_components_defaults );
		...
		$this->add_control(
			new Control(
				...
				[
					...
					'section'         => $this->section,
					'components'      => $components,
					'new_item_fields' => $this->get_new_elements_fields(),
