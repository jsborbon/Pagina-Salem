...
class Blog_Pro_CPT extends Base_Customizer {
	...
	public function adjust_headings() {
		$this->change_customizer_object( 'control', 'neve_' . $this->model->get_archive_type() . '_layout_heading', 'controls_to_wrap', 20 );
	...
	private function add_blog_pro_layout_controls() {
		...
		$this->add_control(
			new Control(
				...
				[
					'sanitize_callback' => 'neve_sanitize_checkbox',
					'default'           => Mods::get( 'neve_blog_separator', ! neve_pro_is_new_skin() ),
				],
				[
					'type'            => 'neve_toggle_control',
					'priority'        => 19,
		 ...
		 */
		$separator_color_default = version_compare( NEVE_VERSION, '2.9.0', '<' ) ? '#f0f0f0' : 'var(--nv-light-bg)';
		$this->add_control(
			new Control(
