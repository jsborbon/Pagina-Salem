...
class Layout_CPT_View extends Base_View {
	...
	final public function register_dynamic_theme_mods() {
		...
		if ( $this->is_blog_pro_enabled ) {
			...
			add_filter(
				'theme_mod_neve_related_posts_columns',
				function ( $value ) {
					if ( is_singular( $this->model->get_type() ) ) {
						return Mods::get( 'neve_related_' . $this->model->get_type() . '_columns', $value );
					}
	...
	public function render_post_meta( $is_list = true ) {
		...
		$has_custom_fields  = Loader::has_compatibility( 'meta_custom_fields' );
		$theme_mod          = $has_custom_fields ? 'neve_single_' . $this->model->get_type() . '_meta_fields' : 'neve_single_' . $this->model->get_type() . '_meta_ordering';
		$default_meta_order = $has_custom_fields ? Layout::get_meta_default_data( 'neve_single_' . $this->model->get_type() . '_meta_ordering', Mods::get( 'neve_single_post_meta_ordering', $default_meta_order ) ) : $default_meta_order;
	...
	private function get_alignment_classes( $theme_mod, $default = array() ) {
		...
		$title_alignment = Mods::get( $theme_mod, $default );
