...
class extract_audios
{
    ...
    public function save_audio($audio_url_prev = "", $found_on = "")
    {
        ...
        if (
            !$this->admin->is_link_exists($audio_url) && !$this->admin->is_failed_file($audio_url)
            ...
            if (!file_exists($my_file)) {
                $abs_url_to_path = $this->admin->abs_url_to_path($audio_url);
                if (strpos($audio_url, $host) !== false && file_exists($abs_url_to_path)){
                    @copy($abs_url_to_path, $my_file);
                }
                else{
                    $data = $this->admin->get_url_data($audio_url);
